...
def render_sync_chart_js(price_df, inflow_df, rolling_window, baseline_window, trend_window, outflow_window, markers_json):
    ...
    <script>
        ...
        const avgSeries = chartI.addLineSeries({{ color: '#3B82F6', lineWidth: 2, lineStyle: LightweightCharts.LineStyle.Solid, priceScaleId: 'right' }});
        avgSeries.setData(aData);
        
        // 3. White Line (Trend)
        const trendSeries = chartI.addLineSeries({{ color: '#FFFFFF', lineWidth: 2, lineStyle: LightweightCharts.LineStyle.Solid, priceScaleId: 'right' }});
        trendSeries.setData(tData);

        // 4. Green Line (Outflow)
        const outflowSeries = chartI.addLineSeries({{ color: '#10B981', lineWidth: 2, lineStyle: LightweightCharts.LineStyle.Solid, priceScaleId: 'right' }});
...
def save_config_callback():
    """Callback to sync widget state to config state and save to disk."""
    # Sync Widget Values -> Config State
    st.session_state['s_resolution'] = st.session_state['w_resolution']
    st.session_state['s_inflow'] = st.session_state['w_inflow']
    st.session_state['s_baseline'] = st.session_state['w_baseline']
    st.session_state['s_trend'] = st.session_state['w_trend']
    st.session_state['s_outflow'] = st.session_state['w_outflow']
    
    # Save to Disk
    settings = {
        's_resolution': st.session_state['s_resolution'],
        's_inflow': st.session_state['s_inflow'],
        's_baseline': st.session_state['s_baseline'],
        's_trend': st.session_state['s_trend'],
        's_outflow': st.session_state['s_outflow']
    }
    try:
...
with st.sidebar:
    ...
    if 'settings_initialized' not in st.session_state:
        config = load_config()
        # Initialize Config State (Source of Truth)
    ...
    with st.expander("⚙️ Advanced Settings", expanded=True):
        ...
        inflow_rolling_window = st.slider(
            "🟠 Inflow Smoothing (min)", 
            min_value=min_win, max_value=60, 
            value=val_in, step=step_win, 
            key="w_inflow",  # Widget Key
            on_change=save_config_callback
        )
        
        baseline_window = st.slider(
            "🔵 Baseline Window (min)", 
            min_value=min_win, max_value=240, 
            value=val_base, step=step_win, 
            key="w_baseline", 
            on_change=save_config_callback
        )
        ...
        trend_window = st.slider(
            ...
            value=val_trend, step=step_win, 
            key="w_trend", 
            on_change=save_config_callback
        )
        
        outflow_window = st.slider(
            "🟢 Outflow Window (min)", 
            min_value=5, max_value=240, 
            value=val_out, step=step_win, 
            key="w_outflow", 
            on_change=save_config_callback
        )
...
if len(date_range) == 2:
    ...
    def get_market_signal_text(df):
        if df.empty: return "NO DATA", "gray", "Waiting for data..."
        state = determine_signal_state(df.iloc[-1])
        
        if state == "SELL":
            return "STRONG SELL", "inverse", "High Inflow vs Baseline! Heavy dumping expected."
        elif state == "BUY":
            return "STRONG BUY", "off", "Outflow Dominance! Whales withdrawing."
        return "NEUTRAL", "off", "Flows are balanced."

    sig_label, sig_color, sig_desc = get_market_signal_text(chart_df)
    
    # Populate Sidebar Metrics
    with signal_container:
        st.metric("AI Market Signal", sig_label, delta=sig_desc, delta_color=sig_color)
